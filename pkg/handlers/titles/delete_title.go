...
func (h handler) DeleteTitle(c *gin.Context) {
	...
	h.DB.Raw(`SELECT id FROM volumes WHERE title_id = ? LIMIT 1`, existingTitleID).Scan(&titleVolumeID)
	if titleVolumeID != 0 {
	...
	if result := h.DB.Exec("DELETE FROM titles WHERE id = ?", existingTitleID); result.Error != nil {
